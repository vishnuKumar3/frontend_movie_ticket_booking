...
export default function Home(){
  ...
  const handleMovieSearch = ()=>{
    ...
    if(pattern.test(searchKeyword)){
      ...
      dispatch(fetchMovies({searchKeyword:searchKeyword, startsFrom:oldDate, startsBefore:currentDate})).then((action:any)=>{
        if(action?.error){
          messageApi.error({content:action?.payload?.message, duration:5})
        }
        else{
  ...
  return(
      <>
        {contextHolder}
        <div className="flex flex-col items-start gap-y-3 p-2 sm:p-5">
          <h2>Trending Movies</h2>
          <div className="flex flex-row items-center sm:gap-x-5 gap-x-1 w-full">
            ...
            <button className="rounded cursor-pointer" style={{border:"none",textTransform:"capitalize",padding:"0px 20px",height:"40px",background:"black",color:"white",fontSize:"15px",fontWeight:700}}><i className="text-white fa fa-filter"></i></button>                                               
          </div>
